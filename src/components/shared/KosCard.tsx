...
// Mapping fasilitas string → ikon SVG path
const fasilitasIcons: Record<string, string> = {
  ...
  dapur: "M6.5 3a.5.5 0 0 0-.5.5v2a.5.5 0 0 0 .5.5h11a.5.5 0 0 0 .5-.5v-2a.5.5 0 0 0-.5-.5h-11zM3 7h18v14H3V7zm2 2v10h14V9H5zm4 0v10",
  ...
  cctv: "M23 7l-7 5 7 5V7zM1 5h15v14H1V5z",
};

function getFasilitasIcon(nama: string): string | null {
  const lower = nama.toLowerCase();
  if (/wi.?fi|internet/i.test(lower)) return fasilitasIcons.wifi;
  if (/\bac\b|pendingin|air.?condition/i.test(lower)) return fasilitasIcons.ac;
  if (/parkir|garage/i.test(lower)) return fasilitasIcons.parkir;
  if (/dapur|kitchen/i.test(lower)) return fasilitasIcons.dapur;
  if (/kasur|tempat.?tidur|bed/i.test(lower)) return fasilitasIcons.kasur;
  if (/lemari|wardrobe/i.test(lower)) return fasilitasIcons.lemari;
  if (/listrik|token/i.test(lower)) return fasilitasIcons.listrik;
  if (/air|shower|kamar.?mandi|km/i.test(lower)) return fasilitasIcons.air;
  if (/\btv\b|televisi/i.test(lower)) return fasilitasIcons.tv;
  if (/cctv|kamera/i.test(lower)) return fasilitasIcons.cctv;
  return null;
}
...
const jenisColors = {
  ...
  campur: "bg-amber-100 text-amber-700",
};
...
export default function KosCard({
  ...
  return (
    <article className="group overflow-hidden rounded-xl border bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
      {/* Foto / Placeholder */}
      <Link href={`/kos/${kos.id}`} className="block relative h-44 overflow-hidden">
        <KosFoto foto={kos.foto} nama={kos.nama} jenis={kos.jenis} className="h-full w-full" />

        {/* Badge jenis kos */}
        <span
          className={`absolute left-3 top-3 rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize ${jenisColors[kos.jenis]}`}
        >
          {kos.jenis}
        </span>
        ...
        {/* Rating badge */}
        {averageRating != null && (
          <span className="absolute right-3 top-3 flex items-center gap-1 rounded-full bg-white/95 px-2 py-0.5 text-xs font-semibold text-amber-600 shadow-sm">
            <svg viewBox="0 0 20 20" className="h-3 w-3 fill-amber-400" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="space-y-3 p-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">
              ...
              {jarak && (
                <span className="ml-1.5 text-xs text-primary/80">• ~{jarak}</span>
              )}
            </p>
        ...
        <div className="flex items-center justify-between gap-3 border-t pt-3">
          ...
          <Link
            href={`/kos/${kos.id}`}
            className="rounded-lg bg-primary px-3 py-1.5 text-sm font-semibold text-white transition hover:bg-primary-600"
          >
            Detail
          </Link>
        </div>
